...
figure,subplot(1,3,1),imshow(uint8(I));title({['Original Image'];[' ']});
subplot(1,3,2),imshow(uint8(IFFT_G));title({['Phase-randomized Version'];[' ']});
subplot(1,3,3),imshow(uint8(Output));title({['Basic Image Features'];['sigma=',num2str(sigma),' epsilon=',num2str(epsilon)]});
